...
interface VehicleCardProps {
  ...
  prearmText?: string
  /** UI G5 (issue #10): renders an explicit "Offline" chip alongside the (necessarily stale) "No heartbeat" pill below. */
  offline?: boolean
}
 ...
 */
export function VehicleCard({ heartbeat, frame, prearmText, offline = false }: VehicleCardProps) {
  const { t } = useTranslation()

  return (
    <div className="flex flex-col rounded-xl border border-nvx-border bg-white p-4 shadow-card">
      <div className="flex items-center justify-between">
        <span className="text-[10.5px] font-extrabold tracking-[.14em] text-nvx-subtle">{t('dashboard.vehicle.title')}</span>
