...
function harness(overrides: { countdownMs?: number; idleLockMs?: number; spinIdleMs?: number; renewMs?: number } = {}) {
  let clock = 0
  const onStop = vi.fn()
  const onRenew = vi.fn()
...
describe('MotorSafety', () => {
  ...
  describe('setSpinning() ready <-> testing', () => {
    ...
    it('setSpinning(true) while already testing updates activeMotors and refreshes lastAct without changing state', () => {
      ...
      expect(h.safety.state).toBe('testing')
      h.advance(4000) // would have tripped auto-stop at t=5000 without the lastAct refresh above
      h.safety.tick()
      expect(h.safety.state).toBe('testing')
