...
- `flight_controller`:固件唯一事实源。其 `scripts/release.sh` 需新增生成 `manifest.json` 一并发布到 GitHub Releases(见 §6)。
...
src/core/mavlink/      generated/  mavgen 生成的 TypeScript 消息定义
...
**选型**:消息定义层用官方 **mavgen 生成 TypeScript**(`minimal + common + ardupilotmega` 子集,vendor 进 `generated/`,可再生成);帧层/会话层自写。排除项:`node-mavlink`(源码依赖 Node `stream`/`crypto`/`Buffer`,不进浏览器 runtime,仅作参考实现);`mavlink-mappings`(npm 发布滞后源码、无 browser/sideEffects 元数据,tree-shaking 不可依赖)只允许作短期原型备胎。
...
6. mavgen TypeScript 生成器成熟度——M1 首周打样验证,备胎为 mavlink-mappings 锁版本
